...
ipcMain.handle('removeTrackedFile', (event, filepath) => {
  ...
  const trackedFiles = LS.get('tracked_files');
  const fileIndex = trackedFiles.findIndex(file => file.filePath === filepath);

  // If the file is being tracked, remove it from the list and stop its watcher
  if (fileIndex >= 0) {
    const fileData = trackedFiles[fileIndex];
    const watcherIndex = watchers.findIndex(watcher => watcher.path === filepath);
    if (watcherIndex >= 0) {
...
ipcMain.handle('openFile', async (event, allowedTypes) => {
  ...
  try {
    ...
    if (!allowedTypes.includes(fileExtension)) {
      ...
      return { error: `Selected file type "${fileExtension}" is not supported.`, newFileData: null };
    }
    ...
    const isAlreadyTracked = trackedFiles.some(file => file.filePath === newFile.filePath);
...
ipcMain.handle('addTrackedFile', (event, fpath_data) => {
  ...
  const fileAlreadyTracked = trackedFiles.some(file => file.filePath === fpath_data.filePath);
...
ipcMain.handle('getChanges', async (event, path) => {
  ...
  const db = new sqlite3.Database(dbPath, (err) => {
    if (err) {
      ...
      console.error(`Error initializing SQLite database for file ${dbPath}: ${err.message}`);
...
function createChangesTable(db, filepath) {
  db.run(`CREATE TABLE IF NOT EXISTS changes (
    ...
    db.get("SELECT COUNT(*) AS count FROM changes", (err, row) => {
      if (err) {
        console.error(`Error checking for records in the changes table for file ${filepath}: ${err.message}`);
        console.error(err.stack);
        return;
      }
      ...
      if (count === 0) {
        console.log(`No records found in the changes table for file ${filepath}, saving initial state`);
...
function startWatcher(filepath) {
  ...
  const watcher = chokidar.watch(filepath, {
    awaitWriteFinish: {
      stabilityThreshold: 5000, // wait 5 seconds after last write event
...
function saveInitialFileState(db, filepath) {
  ...
  fs.stat(filepath, (err, stats) => {
    if (err) throw err;
      ...
      readFileContent(filepath, modifiedTime, (content) => { // pass in modifiedTime to readFileContent
...
function handleFileChange(db, filepath) {
  ...
  fs.stat(filepath, (err, stats) => {
    if (err) {
      console.error(`Error retrieving file stats for file ${filepath}: ${err.message}`);
      return;
    }
...
function readFileContent(filepath, modifiedTime, callback) { // modifiedTime added as parameter
...
function insertChangeIntoDatabase(db, filepath, modifiedTime, content) { // modifiedTime added as parameter
  const dbPath = `${filepath}.db`;
  console.log("inserting into: ", dbPath);
  db.run(`INSERT INTO changes (filePath, modifiedTime, fileContent) VALUES (?, ?, ?)`,
    [filepath, modifiedTime, content], (err) => { // pass in modifiedTime and content as parameters
    if (err) {
      console.error(`Error inserting change into SQLite database for file ${filepath}: ${err.message}`);
    } else {
      console.log(`Inserted change into SQLite database for file ${filepath}`);
    }
  });
}
